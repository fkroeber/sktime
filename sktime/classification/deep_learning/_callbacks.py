"""Callbacks for the neural network classifiers.

The reason for this class is that we rename the logs to be consistent between tf and pytorch.
"""
...
class TfWandbMetricsLogger(keras.callbacks.Callback):
    ...
    * learning rate (both for a fixed value or a learning rate scheduler)
    ...
    def __init__(
        self,
        log_freq: Union[Literal["epoch", "batch"], int] = "epoch",
        ...
        **kwargs: Any,
        ...
        with wandb.sdk.lib.telemetry.context(run=wandb.run) as tel:
        ...
        if self.logging_batch_wise:
            ...
            wandb.define_metric("*", step_metric="batch_step")
        else:
    ...
    def on_epoch_end(self, epoch: int, logs: Optional[Dict[str, Any]] = None) -> None:
        ...
        logs = dict() if logs is None else {f"{k}": v for k, v in logs.items()}
        if "loss" in logs:
            logs["train_loss"] = logs.pop("loss")
        if "accuracy" in logs:
            logs["train_accuracy"] = logs.pop("accuracy")
        if "f1_score" in logs:
            logs["train_f1_score"] = logs.pop("f1_score")
    ...
    def on_batch_end(self, batch: int, logs: Optional[Dict[str, Any]] = None) -> None:
        ...
        if self.logging_batch_wise and batch % self.log_freq == 0:
            logs = {f"{k}": v for k, v in logs.items()} if logs else {}
